...
export class CalendarComponent implements OnInit {
  ...
  ngOnInit(){
    ...
    this._dbservice.get_events().subscribe((events:any[]) => {
      ...
      this.events.map(y=>{
        const TODAY_STR = new Date(y.start).toISOString().replace(/T.*$/, '');
        let obj={
  ...
  constructor(public dialog: MatDialog,private changeDetector: ChangeDetectorRef, private _dbservice: DBService){
  }
  ...
  //  console.log(clickInfo);
   const dialogRef = this.dialog.open(CalenderPopupComponent);
   dialogRef.componentInstance.notice_id = clickInfo.el.fcSeg.eventRange.def.publicId;
  ...
  get_event($event){
    if($event !== null){
      if(this.event){
        ...
        if(!$event){
          this._dbservice.delete_event(this.event.id).subscribe(() => {});
        }
      }
