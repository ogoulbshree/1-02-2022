...
export class TasklistComponent extends DynamicComponent  implements OnInit {
  ...
  this.CAN_ADD = false;
  let result = await this._dbService.getAllTask(1).toPromise();
    let employees = await this._dbService.getAllEmployees().toPromise();
    //console.log("result:", result);
    this.employeeArr= employees["data"];
  ...
  async generateCsv(){
   ...
   exportToPrint() {
    ...
    getEmployeeName = (id) => {
      ...
      for (let k = 0; k < this.employeeArr.length; k++) {
        console.log(this.employeeArr[k].employee_id, id);
  
        if (this.employeeArr[k].employee_id == id)
          return this.employeeArr[k].first_name;
    ...
    searchData(type){
      if(type == 'clear'&& !this.task_name)this.ngOnInit();
      
      else if(type == 'search') 
       this._dbService.getAllGlobalSearchByKeywordAndSection(this.task_name,'task').subscribe((x: any)=>{
