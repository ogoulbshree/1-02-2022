...
export class AddcustomerComponent extends DynamicComponent implements  OnInit {
  ...
  constructor(private _router: Router, private _activatedRoute: ActivatedRoute,injector: Injector, 
    @Inject(LOCALE_ID) public locale: string ,private toast: ToastrService,  private attacSrv: AttachmentService,public _uiservice:UiService, ) 
  {
...
this.notes = res["data"];
  ...
  try {
   ...
   // CustomLogger.logStringWithObject("addedCustomer:result:", result);
    ...
    this._router.navigate(["/helpdesk/customerlist"]);
    this.showError = "";
