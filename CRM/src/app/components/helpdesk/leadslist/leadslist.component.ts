...
*/
  ...
  onClickEdit(obj) {
    this._router.navigate(['/helpdesk/addleads', obj.object_id]);
  }
     ...
     3
      ...
      if (this.flagShowUserReport) {
        ...
        for (let k = 0; k < this.leadDataArr.length; k++) {
          var temp = [  this.leadDataArr[k].object_id,this.leadDataArr[k].first_name,
...
setiFrameForPrint(doc) {
  ...
  searchData(type){ if(type == 'clear' && !this.first_name)this.ngOnInit();
    else if(type == 'search'){
      this._dbService.getAllGlobalSearchByKeywordAndSection(this.first_name,'leads').subscribe((x: any)=>{this.filteredTableDataArr =x.data;
