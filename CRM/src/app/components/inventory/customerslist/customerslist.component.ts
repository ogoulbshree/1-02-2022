...
export class CustomerslistComponent extends DynamicComponent implements OnInit {
  ...
  date = new Date;
   ...
   customerDetailsCount: number = 1;
  filter: { from: number; to: number; };
  customerErrorArray=[];
  
  constructor(private _router: Router,private ngxCsvParser: NgxCsvParser,injector: Injector,public _uiservice:UiService,  private datepipe: DatePipe,
 ...
 */
      ...
      async  onClickDelete(obj,page) {
        if(confirm("Are you sure to delete "+obj.first_name)) {
          ...
          await this.loadPageByNumber(page);
        }
      }
...
filterData() {
  ...
  if (this.model.fromDate == '' || this.model.toDate == '') 
  {
    ...
    this._dbService.getAllGlobalSearchInventoryByDate(this.filter.from,this.filter.to,1).subscribe((response:any)=>{
      ...
      this.filteredTableDataArr = response.data.totalCustomers;
      this.customerDetailsCount = response.data.totalCustomersCount
    ...
    //     this.customerDetailsCount =  this.filteredTableDataArr.length
    //   }
...
getProductDetails(productArr) {
        ...
        fr.onload =  (e) => {
          ...
          this.file_obj = [];
            ...
            for(let i = 1; i < lines.length; i++){
              ...
              if(customers && customers.data.find(x=>x.email == values[3])){
                this.customerErrorArray.push(`Duplicate email ID ${values[3]} found in   `+"row "+i);
  ...
  save(event: Event){
    ...
    //add deals to db
    this._dbService.addMultipleCustomer(this.file_obj).subscribe(async (val) => {
      let result = await this._dbService.objgetAllCustomer(1).toPromise();
      this.customerDataArr = result["results"];
      this.filteredTableDataArr = this.customerDataArr;
      this.customerDetailsCount = result["count"];
