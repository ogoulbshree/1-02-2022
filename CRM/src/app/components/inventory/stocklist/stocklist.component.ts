...
export class StocklistComponent extends DynamicComponent implements OnInit {
  ...
  onClickEdit(obj) {
    this._router.navigate(['inventory/stock', obj.product_id]);
  }
  ...
  search(term: string) {
    ...
    if (this.model.fromDate == '' || this.model.toDate == '') 
    {
      ...
      this._dbService.getAllGlobalSearchInventoryByDate(this.filter.from,this.filter.to,1).subscribe((response:any)=>{
        ...
        this.filteredTableDataArr = response.data.totalProducts;
        this.stackDetailcount = response.data.totalProductsCount
  ...
  downloadFile(data, filename = 'data') {
    if (this.flagShowProductReport) {
      let csvData = this.ConvertToCSV(data, ['code', 'weight','manage_stock',
      'stock_status','stock']);
      //console.log(csvData)
...
searchData(type){
  ...
  if(type == 'clear'&& !this.code)this.ngOnInit();
   ...
   this._dbService.getAllGlobalSearchByKeywordAndSection(this.code,'stock').subscribe((x: any)=>
