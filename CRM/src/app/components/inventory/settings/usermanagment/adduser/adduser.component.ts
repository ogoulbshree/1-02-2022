...
export class AdduserComponent extends DynamicComponent implements OnInit {
 ...
 currentDate$ = of(formatDate(this.currentDate, this.dateFormat, this.locale));
  ...
  async ngOnInit() {
    if(localStorage.getItem("userDetailsInventory"))
    this.newUserArr = (JSON.parse(localStorage.getItem("userDetailsInventory")))
...
async onSubmit() {
  ...
  localStorage.setItem("userDetailsInventory",JSON.stringify(this.newUserArr))
  ...
  //       if(data.data){
          ...
          if(result){
            ...
            this._router.navigate(["/inventory/settings/userlist"]);
