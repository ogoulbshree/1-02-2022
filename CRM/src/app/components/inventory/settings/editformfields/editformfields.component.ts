...
export class EditformfieldsComponent implements OnInit {
  ...
  constructor(private _router: Router, private _dbService: DBService,private _google: GoogleService,
    public _uiservice: UiService ) {
  }

  async ngOnInit() {
      this.componentNameArr = GlobalConstants.INVENTORY.INVENTORY_COMPONENT_NAMES;
  ...
  async translateToAllLanguagesFromEnglish(stringToTranslate) {
    try {   
      this.currentFieldDetail.en = await this.translateIndividualField(stringToTranslate, "en");
