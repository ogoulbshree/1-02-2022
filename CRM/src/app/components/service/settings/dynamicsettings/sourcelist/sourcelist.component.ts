...
export class SourcelistComponent  extends DynamicComponent implements OnInit {
  ...
  constructor( private _router: Router, private _activatedRoute: ActivatedRoute,injector: Injector,public _uiservice: UiService) {
    super(GlobalConstants.SERVICE_COMPONENT_NAME.SALES_SOURCE,injector);
   }
  isUpdate = false;
  ...
  async init() {
    ...
    this.sourceDetailsCount= result["count"];
...
onClickEdit(obj) {
  this._router.navigate(['service/settings/dynamicsettings/source',obj.source_id]);
}
