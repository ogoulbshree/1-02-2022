...
/*  async init() {
  ...
  onClickEdit(obj) {
    this._router.navigate(['/service/settings/adduser', obj.user_id]);
  }
...
filterData() {
  ...
  generateCsv(){
    this.downloadFile(this.userDataArr, 'Service userlist');
    }
    ...
    downloadPdf() {
      let data = null;
      const doc = new jsPDF();
      let fileName = "Service Userlist.pdf";
      if (this.flagShowUserReport) {
        fileName = "Service Userlist.pdf";
        ...
        const header = function (data) {
          ...
          doc.text("Report for Service User List", data.settings.margin.left, 50);
