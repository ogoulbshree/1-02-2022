...
async  onClickDelete(obj) {
  ...
  downloadFile(data, filename = 'data') {
    if (this.flagShowUserReport) {
      let csvData = this.ConvertToCSV(data, [
        'usertype','email','first_name','last_name','phone','dob']);
     // console.log(csvData)
  ...
  generateCsv(){
    this.downloadFile(this.userDataArr, 'Sales userlist');
    }
    ...
    downloadPdf() {
      let data = null;
      const doc = new jsPDF();
      let fileName = "Sales Userlist.pdf";
      if (this.flagShowUserReport) {
        fileName = "Sales Userlist.pdf";
        ...
        const header = function (data) {
          ...
          doc.text("Report for Sales User List", data.settings.margin.left, 50);
  ...
  /* searchData(type){
    if(type == 'clear' && !this.usertype)this.ngOnInit();
    else if(type == 'search')
    this._dbService.getAllGlobalSearchByKeywordAndSection(this.usertype,'usertype').subscribe((x: any)=>{this.filteredTableDataArr =x.data;
      this.userDetailsCount = x.data.length});
