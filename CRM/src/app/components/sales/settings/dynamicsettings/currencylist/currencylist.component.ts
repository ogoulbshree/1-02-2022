...
onClickEdit(obj) {
  this._router.navigate(['sales/settings/dynamicsettings/addcurrency',obj.currency_id]);
}
  ...
  searchData(){
    ...
    this.ngOnInit();
      ...
      this.filteredTableDataArr = this.filteredTableDataArr.filter(res=>
        {
        return res.currency_name.toLocaleLowerCase().match(this.searchText.toLocaleLowerCase());
      });
    }
    }
      ...
      for(let i = 0; i< this.currencyDataArr.length; i++){
       // console.log("this.departmentDataArr[i].created_time::::", this.departmentDataArr[i].created_time);
        if(this.currencyDataArr[i].created_time > filter.from && this.currencyDataArr[i].created_time < filter.to ){
          departmentDataArr.push(this.currencyDataArr[i]);
        }
      }
    }else{
