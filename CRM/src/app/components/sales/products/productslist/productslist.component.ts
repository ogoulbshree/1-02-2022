...
searchData(type){
  ...
  if(type == 'clear' && !this.product_name)this.ngOnInit();
  else if(type == 'search')
  this._dbService.getAllGlobalSearchByKeywordAndSection(this.product_name,'products').subscribe((x: any)=>{this.filteredTableDataArr =x.data;
    this.productDetailsCount = x.data.length});
