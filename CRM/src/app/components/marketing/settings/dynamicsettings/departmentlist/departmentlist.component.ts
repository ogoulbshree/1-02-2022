...
export class DepartmentlistComponent extends DynamicComponent implements OnInit {
  ...
  constructor( private _router: Router, private _activatedRoute: ActivatedRoute,injector: Injector,
    ...
    super(GlobalConstants.COMPONENT_NAME.SALES_DEPARTMENT,injector);
  }
  department_name :any;
...
onClickEdit(obj) {
  this._router.navigate(['marketing/settings/dynamicsettings/department',obj.department_id]);
}
  ...
  /* filterData(){
   ...
   // console.log("this.fromDate::::", this.fromDate);
    ...
    if(this.model.fromDate && this.model.toDate){
      for(let i = 0; i< this.departmentDataArr.length; i++){
        //console.log("this.customerDataArr[i].created_time::::", this.customerDataArr[i].created_time);
        if(this.departmentDataArr[i].created_time > filter.from && this.departmentDataArr[i].created_time < filter.to ){
          departmentDataArr.push(this.departmentDataArr[i]);
        }
      }
