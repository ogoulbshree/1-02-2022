...
export class AdduserComponent extends DynamicComponent implements OnInit {
  ...
  newUserArr= [];
  currentCrudType = GlobalConstants.PARAM_CRUD_TYPE;
 /*  userType: UserType[] = []; */
 ...
 currentDate$ = of(formatDate(this.currentDate, this.dateFormat, this.locale));
  ...
  constructor(private _router: Router, 
    private _activatedRoute: ActivatedRoute, @Inject(LOCALE_ID) public locale: string, injector: Injector,
  ...
  async ngOnInit() {
    if(localStorage.getItem("userDetailsMarketing"))
    this.newUserArr = (JSON.parse(localStorage.getItem("userDetailsMarketing")))
...
async onSubmit() {
  ...
  localStorage.setItem("userDetailsMarketing",JSON.stringify(this.newUserArr))
  ...
  //       if(data.data){
          ...
          if(result){
            ...
            this._router.navigate(["/marketing/settings/userlist"]);
